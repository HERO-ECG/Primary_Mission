...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if((time_piece&time_piece_start)==time_piece_start)
	{
		...
		{		
			...
			BToATest[0]=Strike.vision.prediction.yaw.angular_velocity_vision;//几个调试用的数据
			...
			//电流函数发送
			SetMotoCurrent(&hcan1, STDID_2006_6623, 0, Tripod.Pitch.current_set, 0, Tripod.Yaw.current_set);//6020ID:2,4
