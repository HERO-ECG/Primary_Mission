...
/*******************************************************************************************
  * @Parameter
 *******************************************************************************************/
typedef struct _Common_Pid_t{
	void (*CommonPidSet)(struct _Common_Pid_t *Common_Pid_t, uint32_t pid_mode,
										float max_output, float integral_uplimit, float integral_downlimit,
										float p, float i, float d);
	float (*CommonPidCalc)(struct _Common_Pid_t *pid, float get, float set);
	uint32_t 	 pid_mode;
...
typedef struct __Pid_t{
	...
	void (*PidSet_all)(struct __Pid_t *pid, uint32_t mode, 
		              float max_out, float integral_uplimit, float integral_downlimit,
									float p, float i, float d, 
								  float f, float max_err, float mid_err, float min_err, float deadband, float f_out_limit,float f_divider);
	void (*PidSet_pidf)(struct __Pid_t *pid, float p, float i, float d, float f);
  void (*PidSet_errband)(struct __Pid_t *pid, float max_err, float mid_err, float min_err, float deadband,float f_divider);
