...
void GetEncoderMeasure(Encoder_t *ptr, CAN_HandleTypeDef* hcan){
	if(ptr->init_sta == 0)
	{
		...
		if(ptr->angle - ptr->last_angle > 4096)
			...
			ptr->round_cnt ++;
		ptr->xiangdui_angle = ptr->round_cnt*8192.0f + ptr->angle-ptr->offset_angle;
		ptr->xiangdui_round = ptr->round_cnt + (float)(ptr->angle)/8192.0f-ptr->offset_round; 
		ptr->total_round = ptr->round_cnt + (float)(ptr->angle)/8192.0f; 
		ptr->total_angle = ptr->round_cnt*8192.0f + ptr->angle;
