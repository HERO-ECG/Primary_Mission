...
void TX_DATA_CHASSIS1_ID(CAN_HandleTypeDef* hcan)
{
	float heat_marigin;
	heat_marigin=DJI_ReadData.ext_game_robot_state.shooter_heat1_cooling_limit-DJI_ReadData.ext_power_heat_data.shooter_heat1;
	hcan->pTxMsg->IDE = CAN_ID_EXT;
	hcan->pTxMsg->ExtId = CHASSIS1_ID ;
	hcan->pTxMsg->RTR = CAN_RTR_DATA;
	hcan->pTxMsg->DLC = 0x08;
	hcan->pTxMsg->Data [0] =	(uint8_t)TX2_Data.distance;
	hcan->pTxMsg->Data [1] =	(uint8_t)(((int16_t)(heat_marigin*10))>>8);
	hcan->pTxMsg->Data [2] = 	(uint8_t)(((int16_t)(	TX2_Data.yaw_angle*1000))>>8);
	hcan->pTxMsg->Data [3] = 	(uint8_t)((int16_t)(	TX2_Data.yaw_angle*1000));
	hcan->pTxMsg->Data [4] = 	(uint8_t)(((int16_t)(	TX2_Data.pitch_angle*1000))>>8);
	hcan->pTxMsg->Data [5] = 	(uint8_t)((int16_t)(	TX2_Data.pitch_angle*1000));	
	hcan->pTxMsg->Data [6] = 	(uint8_t)(((int16_t)(heat_marigin*10)));
	hcan->pTxMsg->Data [7] = 	(int8_t)((int16_t)(RoboData.chassis_ctrl.yaw_speed_gyro*1000));
...
void InitRoboData(RoboData_t *RoboData)
{
	...
	Pitch_MOTO[0].MotoParaInit = &MotoParaInit;
	Pitch_MOTO[0].MotoParaInit(&Pitch_MOTO[0]);
	Pitch_MOTO[1].MotoParaInit = &MotoParaInit;
	Pitch_MOTO[1].MotoParaInit(&Pitch_MOTO[1]);
	Yaw_MOTO.MotoParaInit = &MotoParaInit;
	Yaw_MOTO.MotoParaInit(&Yaw_MOTO);

	Friction_Wheel_MOTO[0].MotoParaInit = &MotoParaInit;
	Friction_Wheel_MOTO[0].MotoParaInit(&Friction_Wheel_MOTO[0]);
	Friction_Wheel_MOTO[1].MotoParaInit = &MotoParaInit;
	Friction_Wheel_MOTO[1].MotoParaInit(&Friction_Wheel_MOTO[1]);
	Ctrl_2006_MOTO.MotoParaInit = &MotoParaInit;
	Ctrl_2006_MOTO.MotoParaInit(&Ctrl_2006_MOTO);
	Ammunition_MOTO.MotoParaInit = &MotoParaInit;
  Ammunition_MOTO.MotoParaInit(&Ammunition_MOTO);
	LittleAmmo_MOTO.MotoParaInit = &MotoParaInit;
  LittleAmmo_MOTO.MotoParaInit(&LittleAmmo_MOTO);
	
	
	RC_CtrlData.RCDataParaInit = &RCDataParaInit;
	RC_CtrlData.RCDataParaInit(&RC_CtrlData);
//	TX2_Data.TX2DataParaInit = &TX2DataParaInit;
