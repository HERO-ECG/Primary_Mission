...
void CustomTransmit(CustomData_t CustomData)
{
	...
	while(HAL_UART_Transmit(&huart7, FrameAllMessage, 28, 10) != HAL_OK);
}

void CustomSend(void)//10hz
{
	if(RC_CtrlData.key.key_data.V==0)
	{
		customdata.fdata[0]=Chassis.supercapacitor.Voltage_filtered;
		customdata.fdata[1]=Chassis.supercapacitor.power_percentage*100;
		customdata.fdata[2]=Chassis.power.power_now;
	}
	else
	{
		customdata.fdata[0]=-Chassis.follow.angle;
		customdata.fdata[1]=Tripod.Pitch.angle;
		customdata.fdata[2]=Tripod.Pitch.angle_source.encoder.angle_relativeToChassis;
	}
	
	if(TX2_Data.distance>0)
		customdata.databit.databit.b6=1;
	else
		customdata.databit.databit.b6=0;
	...
	if(Strike.heat_42mm.unlimited==1)
		customdata.databit.databit.b2=1;
	else
		customdata.databit.databit.b2=0;
	
	

//			if(Chassis.follow.mode.now==FRONT_FOLLOW)
//				customdata.fdata[2]=11;
//			else if(Chassis.follow.mode.now==FRONT_NOT_FOLLOW)
//				customdata.fdata[2]=10;
//			else if(Chassis.follow.mode.now==BACK_FOLLOW)
//				customdata.fdata[2]=21;		
//			else if(Chassis.follow.mode.now==BACK_NOT_FOLLOW)
//				customdata.fdata[2]=20;			
//			if(monitor_tx2.status==monitor_regular)
//			{
...
//			{
//				twinkle_counter++;
//				if(twinkle_counter<5)
//					customdata.databit.databit.b1=1;
//				else
//					customdata.databit.databit.b1=0;
//				if(twinkle_counter==10)
//					twinkle_counter=0;
//			}
...
void toEulerAngle(double quat_fp32[4], double *roll, double *pitch, double *yaw)
{
	...
	sinr_cosp = +2.0 * (quat_fp32[0]*quat_fp32[1] + quat_fp32[2]*quat_fp32[3]);
	cosr_cosp = +1.0 - 2.0 * (quat_fp32[1]*quat_fp32[1] + quat_fp32[2]*quat_fp32[2]);
	*roll = atan2(sinr_cosp, cosr_cosp)*180/3.1415926535897f;

	// pitch (y-axis rotation)
	double sinp;
	sinp = +2.0 * (quat_fp32[0]*quat_fp32[2] - quat_fp32[3]*quat_fp32[1]);
	if (fabs(sinp) >= 1)
	*pitch = copysign(3.1415926535897f / 2, sinp); // use 90 degrees if out of range
	else
	*pitch = asin(sinp)*180/3.1415926535897f;

	// yaw (z-axis rotation)
	double siny_cosp,cosy_cosp;
	siny_cosp = +2.0 * (quat_fp32[0]*quat_fp32[3] + quat_fp32[1]*quat_fp32[2]);
	cosy_cosp = +1.0 - 2.0 * (quat_fp32[2]*quat_fp32[2] + quat_fp32[3]*quat_fp32[3]); 
	*yaw = atan2(siny_cosp, cosy_cosp)*180/3.1415926535897f;
}
